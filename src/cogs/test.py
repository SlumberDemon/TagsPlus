...
class Test(commands.Cog):
    ...
    async def datacreate(self, ctx, name, *, content: str):
        ...
        await guild_create_tag(guild_id=ctx.guild.id, item=[{"owner": owner, "name": tag_name, "content": content, "created_at": int(time)}], owner=owner, name=tag_name)
    ...
    async def datacreatesingle(self, ctx, name, *, content: str):
        ...
        owner = f'{ctx.author.id}'
        tag_name = name.replace(' ', '_')
        await guild_create_tag(guild_id=ctx.guild.id, item=str(time), owner=owner, name=tag_name)
        await ctx.send(f'Tag `{name}` successfully created.')
